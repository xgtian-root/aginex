...
import (
	...
	scaffoldassets "github.com/xgtian-root/aginex"
...
const (
	frameworkModulePath = "github.com/xgtian-root/aginex"
	scaffoldSchema      = 1
	scaffoldTemplate    = 1
)
	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			if err != nil && localAginexPath != "" && errors.Is(err, errUnpublishedFrameworkBuild) {
				version, err = normalizeFrameworkVersion(buildinfo.Version)
			}
			if err != nil {
...
func renderScaffold(root string, assets fs.FS, model scaffoldModel) error {
	...
	generated := map[string]string{
		...
		"cmd/openapi/main.go":                projectOpenAPIMain,
		"cmd/server/main.go":                 projectServerMain,
		"cmd/worker/main.go":                 projectWorkerMain,
		"internal/composition/definition.go": projectComposition,
	}
...
func copyScaffoldAssets(root string, assets fs.FS, model scaffoldModel) error {
	return fs.WalkDir(assets, ".", func(assetPath string, entry fs.DirEntry, walkErr error) error {
		...
		content, err = customizeScaffoldAsset(assetPath, content, model)
		if err != nil {
			return err
		}
...
func projectGoSum(frameworkGoSum []byte) []byte {
	...
	lines := strings.Split(string(frameworkGoSum), "\n")
	kept := make([]string, 0, len(lines))
	for _, line := range lines {
		fields := strings.Fields(line)
		if len(fields) > 0 {
			if _, excluded := cliOnly[fields[0]]; excluded {
				continue
			}
		}
...
func projectGoMod(frameworkGoMod []byte, model scaffoldModel) ([]byte, error) {
	...
	// These dependencies are used by the Aginex CLI, not by the public
	// application runtime. Every other bundled dependency is retained as
	// indirect so a fresh project is buildable before its first `go mod tidy`.
	cliOnly := map[string]struct{}{
		"github.com/inconshreveable/mousetrap": {},
		"github.com/spf13/cobra":               {},
		"github.com/spf13/pflag":               {},
	...
	for _, requirement := range parsed.Require {
		if _, excluded := cliOnly[requirement.Mod.Path]; excluded {
			continue
		}
...
func scaffoldFileManifest(root string) ([]manifestFile, error) {
	...
	err := filepath.WalkDir(root, func(filePath string, entry fs.DirEntry, walkErr error) error {
		...
		if relative == "docs/openapi.json" || relative == "apps/web/lib/api.generated.ts" {
			ownership = "generated"
		}
...
func bundledFrameworkVersion() (string, error) {
	...
	return resolveFrameworkBuildVersion(mainVersion, buildinfo.Version, modified)
}
...
func resolveFrameworkBuildVersion(mainVersion, declaredVersion string, modified bool) (string, error) {
	...
	if !dirty && mainVersion != "" && mainVersion != "(devel)" {
		return normalizeFrameworkVersion(mainVersion)
	}
...
import "github.com/xgtian-root/aginex/framework/application"
...
uses PostgreSQL jobs and runs ` + "`go run ./cmd/worker`" + ` after the API is ready.

Application modules are selected in ` + "`internal/composition/definition.go`" + `.
The Aginex framework version is pinned in ` + "`go.mod`" + `, while
...
` + "`replace`" + ` directive in ` + "`go.mod`" + `. Remove that directive and pin a published
