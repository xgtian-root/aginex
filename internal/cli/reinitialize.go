...
func newReinitializeCommand(dependencies reinitializeDependencies) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			fmt.Fprintln(cmd.OutOrStdout(), "Run `go run ./cmd/aginex dev` and complete browser Setup with a new administrator.")
			return nil
		},
	}
...
func printReinitializePlan(command *cobra.Command, plan reinitializePlan) {
	...
	fmt.Fprintf(
		command.OutOrStdout(),
		"  go run ./cmd/aginex dev reinitialize --config %s --confirm %s\n",
