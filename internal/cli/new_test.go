...
func TestNewCommandUsesExplicitGoModulePath(t *testing.T) {
	...
	server := readTestFile(t, filepath.Join(target, "cmd", "server", "main.go"))
	if !bytes.Contains(
		server,
...
func TestNewCommandUsesExplicitLocalAginexCheckout(t *testing.T) {
	...
	if err := os.Mkdir(localAginex, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(localAginex, "go.mod"),
	...
	goModPath := filepath.Join(workingDirectory, "local-app", "go.mod")
	...
	if len(parsed.Replace) != 1 ||
		...
		parsed.Replace[0].New.Path != filepath.ToSlash(canonicalLocalAginex) {
...
func TestResolveFrameworkBuildVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "installed published prerelease",
			mainVersion:     "v1.3.0-dev.1",
			declaredVersion: "0.1.0-dev",
			want:            "v1.3.0-dev.1",
		},
		{
...
func TestNewCommandRejectsNonAginexLocalCheckoutWithoutMutation(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(wrongCheckout, "go.mod"),
...
func TestGeneratedProjectBuildsAsExternalConsumer(t *testing.T) {
	...
	if len(parsed.Replace) != 1 ||
		parsed.Replace[0].Old.Path != frameworkModulePath ||
		parsed.Replace[0].New.Path != filepath.ToSlash(canonicalRepositoryRoot) {
	...
	for _, arguments := range [][]string{
		...
		command.Env = append(
			os.Environ(),
			"GOCACHE="+filepath.Join(workingDirectory, "go-build-cache"),
			"GOPROXY=off",
			"GOSUMDB=off",
			"GOWORK=off",
		)
...
func TestNewCommandManifestIsDeterministicAndMatchesFileHashes(t *testing.T) {
	...
	for _, file := range manifest.Files {
		...
		if file.Path == "docs/openapi.json" || file.Path == "apps/web/lib/api.generated.ts" {
			ownership = "generated"
		}
...
func minimalScaffoldAssets() fs.FS {
	return fstest.MapFS{
		...
		"go.mod": &fstest.MapFile{Data: []byte(`module github.com/xgtian-root/aginex
