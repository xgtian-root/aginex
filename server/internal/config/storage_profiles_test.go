...
func TestValidateStorageAccessBaseURL(t *testing.T) {
	for _, test := range []struct {
		name      string
		value     string
		wantError bool
	}{
		{name: "official bucket origin", value: "https://images.oss-cn-hangzhou.aliyuncs.com"},
		{name: "custom CNAME", value: "https://files.example.com"},
		{name: "HTTP", value: "http://files.example.com", wantError: true},
		{name: "path", value: "https://files.example.com/private", wantError: true},
		{name: "query", value: "https://files.example.com?token=secret", wantError: true},
		{name: "credentials", value: "https://user:secret@files.example.com", wantError: true},
		{name: "missing", wantError: true},
	} {
		t.Run(test.name, func(t *testing.T) {
			err := ValidateStorageAccessBaseURL(test.value)
			if (err != nil) != test.wantError {
				t.Fatalf("error = %v, wantError %v", err, test.wantError)
			}
		})
