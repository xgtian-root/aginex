...
func TestValidateRequiresOSSAccessBaseURL(t *testing.T) {
	cfg := WithDefaults(Config{
		Environment: "test",
		Database:    Database{Driver: "sqlite", DSN: "test.db"},
		Jobs:        Jobs{Driver: "disabled"},
		Storage: Storage{
			Driver: "oss", Bucket: "images", Region: "cn-hangzhou",
			AccessKeyID: "key", AccessKeySecret: "secret",
		},
	})
	if err := Validate(cfg); err == nil || !strings.Contains(err.Error(), "access base URL") {
		t.Fatalf("missing OSS access URL error = %v", err)
	}
	cfg.Storage.AccessBaseURL = "https://files.example.com/"
	if err := Validate(cfg); err != nil {
		t.Fatalf("custom OSS access URL error = %v", err)
	}
	if cfg = WithDefaults(cfg); cfg.Storage.AccessBaseURL != "https://files.example.com" {
		t.Fatalf("normalized access URL = %q", cfg.Storage.AccessBaseURL)
	}
}
