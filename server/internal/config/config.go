...
func WithDefaults(cfg Config) Config {
	...
	cfg.Storage.AccessBaseURL = strings.TrimRight(strings.TrimSpace(cfg.Storage.AccessBaseURL), "/")
...
func validateWithoutDatabase(cfg Config) error {
	...
	if cfg.Storage.Driver == "oss" {
		if err := ValidateStorageAccessBaseURL(cfg.Storage.AccessBaseURL); err != nil {
			return err
		}
	} else if strings.TrimSpace(cfg.Storage.AccessBaseURL) != "" {
		return fmt.Errorf("AGINEX_STORAGE_ACCESS_BASE_URL is only valid with AGINEX_STORAGE_DRIVER=oss")
	}
