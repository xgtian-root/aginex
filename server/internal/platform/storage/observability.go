...
type observedVerifiedPresentationStorage struct {
	observer *observedStorage
	next     VerifiedPresentationFinalizer
}
...
// AsVerifiedPresentationFinalizer resolves verified metadata promotion while
// preserving telemetry across transparent storage decorators.
func AsVerifiedPresentationFinalizer(store Storage) (VerifiedPresentationFinalizer, bool) {
	for range maxStorageUnwrapDepth {
		if nilStorage(store) {
			return nil, false
		}
		switch observed := store.(type) {
		case *observedReadyStorage:
			if next, ok := observed.next.(VerifiedPresentationFinalizer); ok {
				return &observedVerifiedPresentationStorage{observer: observed.observedStorage, next: next}, true
			}
		case *observedStorage:
			if next, ok := observed.next.(VerifiedPresentationFinalizer); ok {
				return &observedVerifiedPresentationStorage{observer: observed, next: next}, true
			}
		default:
			if finalizer, ok := store.(VerifiedPresentationFinalizer); ok {
...
func (storage *observedVerifiedPresentationStorage) FinalizeVerifiedPresentation(
	ctx context.Context,
	request VerifiedPresentationRequest,
) (ObjectInfo, error) {
	ctx, observation := storage.observer.begin(ctx, "finalize_verified_presentation")
	result, err := storage.next.FinalizeVerifiedPresentation(ctx, request)
	bytes := int64(-1)
	if err == nil {
		bytes = result.Size
	}
