...
func FromConfig(
	...
	case "oss":
		...
		if err := config.ValidateStorageAccessBaseURL(cfg.AccessBaseURL); err != nil {
			return nil, err
		}
		readCfg := ossCfg.Copy()
		readCfg.WithEndpoint(cfg.AccessBaseURL).WithUseCName(true)
		return NewOSSWithReadConfig(ossCfg, &readCfg, cfg.Bucket, policy), nil
	default:
