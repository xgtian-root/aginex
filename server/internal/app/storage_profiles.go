...
func (a *App) storageProfileResponse(ctx context.Context, document storageDocument, profile config.StorageProfile) StorageProfileResponse {
	...
	return StorageProfileResponse{
		...
		Endpoint: effective.Endpoint, AccessBaseURL: profile.AccessBaseURL,
		AccountID: profile.AccountID, LocalRoot: profile.LocalRoot,
...
func storageIdentity(profile config.StorageProfile) string {
	return strings.Join([]string{string(profile.Provider), profile.Bucket, profile.Region, profile.Endpoint, profile.AccessBaseURL, profile.AccountID, profile.LocalRoot}, "\x00")
}
...
func (a *App) profileFromRequest(input StorageProfileRequest, current *config.StorageProfile) (config.StorageProfile, error) {
	...
	profile.AccessBaseURL = strings.TrimRight(strings.TrimSpace(input.AccessBaseURL), "/")
	...
	case config.StorageProviderAlibabaOSS:
		...
		if err := config.ValidateStorageAccessBaseURL(profile.AccessBaseURL); err != nil {
			return config.StorageProfile{}, err
		}
	case config.StorageProviderAWSS3:
		if profile.Endpoint != "" || profile.AccessBaseURL != "" || profile.AccountID != "" {
			return config.StorageProfile{}, errors.New("AWS S3 endpoint is resolved by the SDK")
		}
	case config.StorageProviderMinIO:
		if profile.AccessBaseURL != "" {
			return config.StorageProfile{}, errors.New("access base URL is only valid for Alibaba Cloud OSS")
		}
		if profile.Region == "" {
...
func (a *App) checkStorageReadiness(ctx context.Context, profile config.StorageProfile) error {
	...
	if browser, ok := candidate.(storage.BrowserUploadReadinessChecker); ok {
		browserCtx, browserCancel := context.WithTimeout(ctx, storageReadinessTimeout)
		defer browserCancel()
		return browser.CheckBrowserUploadReadiness(
			browserCtx,
			a.cfg.AllowedWebOrigins(),
		)
	}
	return nil
}
...
func writeStorageReadinessProblem(c *gin.Context, err error) {
	...
	if errors.Is(err, storage.ErrBrowserUploadCORS) {
		writeProblem(c, http.StatusBadRequest, "Storage CORS is not ready", "Configure the bucket CORS rule for every administration origin, allow PUT and signed request headers, and expose ETag.")
		return
	}
