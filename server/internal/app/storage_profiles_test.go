...
func TestStorageProfileProviderPresets(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			request:  StorageProfileRequest{Name: "OSS", Provider: "aliyun-oss", Bucket: "images", Region: "cn-hangzhou", Endpoint: "https://oss-cn-hangzhou.aliyuncs.com", AccessBaseURL: "https://images.oss-cn-hangzhou.aliyuncs.com", AccessKeyID: &accessKeyID, AccessKeySecret: &accessKeySecret},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.driver == "oss" && storage.AccessBaseURL != "https://images.oss-cn-hangzhou.aliyuncs.com" {
				t.Fatalf("OSS access base URL = %q", storage.AccessBaseURL)
			}
		})
	}
}
