...
func (a *App) completeUploadSession(c *gin.Context) {
	...
	if err != nil {
		logRequestFailure(c, "storage_finalize_completed_presentation", err)
		writeProblem(c, http.StatusServiceUnavailable, "Upload could not be finalized", "Retry completion after the storage provider becomes available.")
		return
	}
	principal := currentPrincipal(c)
	err = a.writes.Run(c.Request.Context(), func(tx *gorm.DB) (frameworkaudit.Event, error) {
