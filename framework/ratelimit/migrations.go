...
const (
	// MigrationTableName isolates this optional module's migration history from
	// the application's core Goose history.
	MigrationTableName = "aginex_rate_limit_migrations"
	latestMigration    = int64(1)
)

// ErrSchemaNotCurrent means the limiter tables were not installed by the
// explicit migration process or are behind this package.
var ErrSchemaNotCurrent = errors.New("rate limit schema is not current")
...
// NewMigrationProvider returns a Goose provider for this opt-in module. The
// caller owns execution timing and should invoke it only from an explicit
// migrate process.
func NewMigrationProvider(db *sql.DB, dialect string) (*goose.Provider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open %s rate limit migrations: %w", dialect, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("configure %s rate limit migrations: %w", dialect, err)
	}
	return provider, nil
}

// Status reads the optional module's migration state without creating tables.
func Status(ctx context.Context, db *sql.DB, dialect string) (MigrationStatus, error) {
	...
	if err := db.QueryRowContext(ctx, existsQuery, MigrationTableName).Scan(&count); err != nil {
		return MigrationStatus{}, fmt.Errorf("check %s rate limit migration table: %w", dialect, err)
	}
	...
	`).Scan(&current); err != nil {
		return MigrationStatus{}, fmt.Errorf("read %s rate limit migration version: %w", dialect, err)
	}
	if current.Valid {
...
// EnsureCurrent verifies the module schema without mutating it.
...
func migrationOptions(dialect string) ([]goose.ProviderOption, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("configure postgres rate limit migration lock: %w", err)
	}
